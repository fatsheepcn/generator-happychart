define([
  ...
  <% if(!list_datepicker || list_datepicker != false){ %>
    ...
    // set default datePicker params
    $scope.datePicker = {
      start: {
        dt: dateUtil.getRelativeDate(-7, new Date())
      },
      end: {
    ...
    // set default apiParams
    apiParams.min_time = _dateFormat($scope.datePicker.start.dt);
    apiParams.max_time = _dateFormat($scope.datePicker.end.dt);
